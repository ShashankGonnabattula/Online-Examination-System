...
function TeacherLayout() {
  ...
  const item = (to, label, icon) => (
    <NavLink to={to} className={({ isActive }) => `flex items-center gap-3 px-4 py-3 text-sm ${isActive ? 'bg-primary-500/20 text-primary-300' : 'text-gray-300 hover:bg-white/10'}`} end>
      <Icon>{icon}</Icon>
      <span>{label}</span>
    </NavLink>
  )
